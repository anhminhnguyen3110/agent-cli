...
agentscli --agent mybot

# Use a specific model (auto-detects provider)
agentscli --model claude-sonnet-4-5-20250929
agentscli --model gpt-4o

# Auto-approve tool usage (skip human-in-the-loop prompts)
agentscli --auto-approve

# Execute code in a remote sandbox
agentscli --sandbox modal        # or runloop, daytona
agentscli --sandbox-id dbx_123   # reuse existing sandbox
```
...
Each agent has its own configuration directory at `~/.agentscli/<agent_name>/`, with default `agent`.
...
**Agent Tracing** - Traces agentscli operations (tool calls, agent decisions):

```bash
export AGENTSCLI_LANGSMITH_PROJECT="my-agent-project"
```
...
export AGENTSCLI_LANGSMITH_PROJECT="agent-traces"
...
```
✓ LangSmith tracing enabled: AgentsCLI → 'agent-traces'
...
If `AGENTSCLI_LANGSMITH_PROJECT` is not set, both agent and user code trace to the same project specified by `LANGSMITH_PROJECT`.

## Customization

There are two primary ways to customize any agent: **memory** and **skills**.

Each agent has its own global configuration directory at `~/.agentscli/<agent_name>/`:

```
~/.agentscli/<agent_name>/
...
- Project-specific `AGENTS.md` from `[project-root]/.agentscli/AGENTS.md`
- Project-specific skills from `[project-root]/.agentscli/skills/`
...
**Global `AGENTS.md`** (`~/.agentscli/agent/AGENTS.md`)
...
**Project `AGENTS.md`** (`.agentscli/AGENTS.md` in project root)
...
Beyond `AGENTS.md`, you can create additional memory files in `.agentscli/` for structured project knowledge. These work similarly to [Anthropic's Memory Tool](https://platform.claude.com/docs/en/agents-and-tools/tool-use/memory-tool). The agent receives instructions on when to read and update these files.

**How it works:**

1. Create markdown files in `[project-root]/.agentscli/` (e.g., `api-design.md`, `architecture.md`, `deployment.md`)
...
```bash
mkdir -p ~/.agentscli/agent/skills
cp -r examples/skills/web-research ~/.agentscli/agent/skills/
```
...
1. **At startup** - SkillsMiddleware scans `~/.agentscli/agent/skills/` and `.agentscli/skills/` directories
