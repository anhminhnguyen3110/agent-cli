...
from agentscli import create_deep_agent
from agentscli.backends import CompositeBackend
from agentscli.backends.filesystem import FilesystemBackend
from agentscli.backends.sandbox import SandboxBackendProtocol
from agentscli.middleware import MemoryMiddleware, SkillsMiddleware
...
from agentscli_cli.config import COLORS, config, console, get_default_coding_instructions, settings
from agentscli_cli.integrations.sandbox_factory import get_default_working_dir
from agentscli_cli.shell import ShellMiddleware
from agentscli_cli.mcp_loader_custom import load_mcp_tools_custom, find_mcp_config


def list_agents() -> None:
    """List all available agents."""
    agents_dir = settings.user_agentscli_dir

    if not agents_dir.exists() or not any(agents_dir.iterdir()):
        console.print("[yellow]No agents found.[/yellow]")
        console.print(
            "[dim]Agents will be created in ~/.agentscli/ when you first use them.[/dim]",
            style=COLORS["dim"],
        )
        return
...
def get_system_prompt(assistant_id: str, sandbox_type: str | None = None) -> str:
    ...
    """
    agent_dir_path = f"~/.agentscli/{assistant_id}"
...
def create_cli_agent(
    ...
    *,
    ...
    This is the main entry point for creating a agentscli CLI agent, usable both
    ...
    if sandbox is None:
        # ========== LOCAL MODE ==========
        backend = FilesystemBackend(virtual_mode=False)  # Allow absolute paths on Windows
