...
- agentscli skills list --agent <agent> [--project]
- agentscli skills create <name>
- agentscli skills info <name>
"""
...
from agentscli_cli.config import COLORS, Settings, console
from agentscli_cli.skills.load import list_skills
...
def _list(agent: str, *, project: bool = False) -> None:
    ...
    if project:
        ...
        if not project_skills_dir.exists() or not any(project_skills_dir.iterdir()):
            ...
            console.print(
                "\n[dim]Create a project skill:\n  agentscli skills create my-skill --project[/dim]",
                style=COLORS["dim"],
            )
            return
        ...
        if not skills:
            ...
            console.print(
                "[dim]Skills will be created in ~/.agentscli/agent/skills/ when you add them.[/dim]",
                style=COLORS["dim"],
            )
            console.print(
                "\n[dim]Create your first skill:\n  agentscli skills create my-skill[/dim]",
                style=COLORS["dim"],
            )
            return
...
# license: Apache-2.0
# compatibility: Designed for agentscli CLI
# metadata:
#   author: your-org
#   version: "1.0"
...
"""
    ...
    console.print(
        ...
        f"  nano {skill_md}\n"
        "\n"
        "💡 See examples/skills/ in the agentscli repo for example skills:\n"
        "   - web-research: Structured research workflow\n"
        "   - langgraph-docs: LangGraph documentation lookup\n"
        "\n"
        "   Copy an example: cp -r examples/skills/web-research ~/.agentscli/agent/skills/\n",
        style=COLORS["dim"],
    )
...
def execute_skills_command(args: argparse.Namespace) -> None:
    ...
    if args.skills_command == "list":
        ...
        console.print("  agentscli skills <command> [options]\n")
        ...
        console.print("  agentscli skills list")
        console.print("  agentscli skills create web-research")
        console.print("  agentscli skills info web-research")
        console.print("\n[dim]For more help on a specific command:[/dim]", style=COLORS["dim"])
        console.print("  agentscli skills <command> --help", style=COLORS["dim"])
