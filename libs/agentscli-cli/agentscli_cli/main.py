"""Main entry point and CLI loop for agentscli."""
# ruff: noqa: T201
...
from agentscli_cli.agent import create_cli_agent, list_agents, reset_agent

# CRITICAL: Import config FIRST to set LANGSMITH_PROJECT before LangChain loads
from agentscli_cli.config import (
    console,
    create_model,
    settings,
)
from agentscli_cli.integrations.sandbox_factory import create_sandbox
from agentscli_cli.sessions import (
...
from agentscli_cli.skills import execute_skills_command, setup_skills_parser
from agentscli_cli.tools import fetch_url, http_request, web_search
from agentscli_cli.ui import show_help
...
def check_cli_dependencies() -> None:
    ...
    if missing:
        ...
        print("\nThe following packages are required to use the agentscli CLI:")
        for pkg in missing:
            print(f"  - {pkg}")
        print("\nPlease install them with:")
        print("  pip install agentscli[cli]")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="AgentsCLI - AI Coding Assistant",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"agentscli {__version__}",
    )
...
async def run_textual_cli_async(
    ...
    *,
    ...
    """
    from agentscli_cli.app import run_textual_app
...
def cli_main() -> None:
    ...
    # This ensures agent traces → AGENTSCLI_LANGSMITH_PROJECT
    ...
    try:
        ...
        if args.command == "help":
            ...
            if args.threads_command == "list":
                ...
                console.print("[yellow]Usage: agentscli threads <list|delete>[/yellow]")
        else:
            ...
            if args.resume_thread == "__MOST_RECENT__":
                ...
                if asyncio.run(thread_exists(args.resume_thread)):
                    ...
                    console.print(
                        "[dim]Use 'agentscli threads list' to see available threads.[/dim]"
                    )
                    sys.exit(1)
