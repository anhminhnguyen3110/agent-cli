...
from agentscli_cli._version import __version__

# Load .env from multiple locations (for end users with wheel installation)
# Priority: ~/.agentscli/.env > ./.env
user_env = Path.home() / ".agentscli" / ".env"
if user_env.exists():
...
_deepagents_project = os.environ.get("AGENTSCLI_LANGSMITH_PROJECT")
...
def _find_project_agent_md(project_root: Path) -> list[Path]:
    ...
    1. project_root/.agentscli/AGENTS.md
    ...
    # Check .agentscli/AGENTS.md (preferred)
    agentscli_md = project_root / ".agentscli" / "AGENTS.md"
    if agentscli_md.exists():
        paths.append(agentscli_md)
...
@dataclass
class Settings:
    """Global settings and environment detection for agentscli-cli.
    ...
    Attributes:
        ...
        agentscli_langchain_project: LangSmith project name for agentscli agent tracing
    ...
    agentscli_langchain_project: str | None  # For agentscli agent tracing
    ...
    def from_environment(cls, *, start_path: Path | None = None) -> "Settings":
        ...
        # AGENTSCLI_LANGSMITH_PROJECT: Project for agentscli agent tracing
        ...
        agentscli_langchain_project = os.environ.get("AGENTSCLI_LANGSMITH_PROJECT")
        ...
        return cls(
            ...
            agentscli_langchain_project=agentscli_langchain_project,
    ...
    @property
    def has_agentscli_langchain_project(self) -> bool:
        """Check if agentscli LangChain project name is configured."""
        return self.agentscli_langchain_project is not None
    ...
    @property
    def user_agentscli_dir(self) -> Path:
        """Get the base user-level .agentscli directory.

        Returns:
            Path to ~/.agentscli
        """
        return Path.home() / ".agentscli"
    ...
    def get_user_agent_md_path(self, agent_name: str) -> Path:
        ...
        Returns:
            Path to ~/.agentscli/{agent_name}/AGENTS.md
        """
        return Path.home() / ".agentscli" / agent_name / "AGENTS.md"
    ...
    def get_project_agent_md_path(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.agentscli/AGENTS.md, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".agentscli" / "AGENTS.md"
    ...
    def get_agent_dir(self, agent_name: str) -> Path:
        ...
        return Path.home() / ".agentscli" / agent_name
    ...
    def ensure_project_agentscli_dir(self) -> Path | None:
        """Ensure the project .agentscli directory exists and return its path.

        Returns:
            Path to project .agentscli directory, or None if not in a project
        """
        if not self.project_root:
            return None

        project_agentscli_dir = self.project_root / ".agentscli"
        project_agentscli_dir.mkdir(parents=True, exist_ok=True)
        return project_agentscli_dir
    ...
    def get_user_skills_dir(self, agent_name: str) -> Path:
        ...
        Returns:
            Path to ~/.agentscli/{agent_name}/skills/
        """
    ...
    def get_project_skills_dir(self) -> Path | None:
        ...
        Returns:
            Path to {project_root}/.agentscli/skills/, or None if not in a project
        """
        if not self.project_root:
            return None
        return self.project_root / ".agentscli" / "skills"
